...
def add_store(request : Request):
    ...
    new_store = store(title=name, description=desc, rating=rating, pub_date=pub_date)
    new_store.save()
...
def list_stores(request : Request):
    ...
    print(store.objects.count()) #returns the total count of stores in the database, for testing only

    stores_with_high_rating_count = store.objects.filter(rating__gte=4).count() #get count of stores greater thank 4
    ...
    if "search" in request.query_params:
        ...
        all_stores = store.objects.filter(title__startswith=search_phrase)[skip:get]
    else:
        all_stores = store.objects.all().order_by('-rating')[skip:get]
